...
die "usage: evaluate-modified.pl <out-dir> <chunk-dir> <extension> <0/1:internal-only> [program-name]" unless @ARGV>=4;

my $genezilla=shift @ARGV;
my $cdna=shift @ARGV;
my $ext=shift @ARGV;
my $INTERNAL_ONLY=0+shift @ARGV;

my $programName="         ";
if(@ARGV==1) {$programName=shift @ARGV}

my $gffReader=new GffTranscriptReader;
my $genezillaFiles=getFiles($genezilla,$ext);
...
#	next unless -e "$programDir/$file";
	...
	#undef $cDNAs; undef $cdnaExons; undef $cdnaSplices; undef $cdnaCodons; undef $predExons; undef $predSplices; undef $predCodons; undef $commonSplices; undef $commonCodons;
      }
...
#----------------------------------------------------------------
sub keepInternalExonsOnly {
  my ($transcript)=@_;
  my $numExons=$transcript->numExons();
  for(my $i=0 ; $i<$numExons ; ++$i) {
    my $exon=$transcript->getIthExon($i);
    next if($exon->getType() eq "internal_exon");
