#!/usr/bin/perl
#Merge GFF annotations into one big file, and translate internal coords
#to real coords. Also, ignore all initial and final exons.
#
...
die "merge-annotations-onlyinternal.pl <chunk-directory> [-comments]" unless @ARGV == 1 || @ARGV == 2;
...
for my $gff (@gffList)
{
  ...
  while(<GFF>)
  {
    ...
    if ($currentLineSplit[2] =~ /initial/ || $currentLineSplit[2] =~ /final/)
    {
      next;
    }
