...
die "usage: evaluate-modified.pl <out-dir> <chunk-dir> [program-name]" unless @ARGV>=2;

my $genezilla=shift @ARGV;
my $cdna=shift @ARGV;

my $programName="Decoder  ";
if(@ARGV==1) {$programName=shift @ARGV}

my $gffReader=new GffTranscriptReader;
#my $genezillaFiles=getFiles($genezilla,"gff");
my $genezillaFiles=getFiles($genezilla,"nbest");
#my $genezillaFiles=getFiles($genezilla,"genezilla");
#my $genezillaFiles=getFiles($genezilla,"abinitio");
my $cdnaFiles=getFiles($cdna,"gff");
...
sub evaluateNuc
  {
    ...
    die unless $numFiles>0;
    for(my $i=0 ; $i<$numFiles ; ++$i)
      {
	my $file=$files->[$i];
	$file=~/(\d+)/ || next;
	my $cdnaFile="$1.gff";
	next unless -e "$programDir/$file";
...
sub evaluateExons
  {
	...
	dropSuperfluousFeatures($cDNAs,$leftTerminus,$rightTerminus);
	next unless @$cDNAs>0;
...
sub evaluateGenes
  {
    ...
    print STDOUT "|$percent\% ($totalIdentical) N=$numFiles\n";
  }
#----------------------------------------------------------------
sub getSpliceSites
  {
